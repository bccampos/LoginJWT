...
namespace SmartStore.Identity.API.Controllers
{
    ...
    public class AuthController : MainController
    {
        ...
        public async Task<ActionResult> Register(UserRegister userRegister)
        {
            ...
            {
                AddErrorList(error.Description);
            }
        ...
        public async Task<ActionResult> Login(UserLogin userLogin)
        {
            ...
            {
                AddErrorList("User temporarily blocked by invalid attempts");
                return CustomResponse();
            }

            AddErrorList("Incorrect username or password");
        ...
        private async Task<ClaimsIdentity> GetUserClaims(ICollection<Claim> claims, IdentityUser user)
        {
            ...
            claims.Add(new Claim(JwtRegisteredClaimNames.Nbf, ToUnixEpochDate(DateTime.UtcNow).ToString())); //Expired Date
            claims.Add(new Claim(JwtRegisteredClaimNames.Iat, ToUnixEpochDate(DateTime.UtcNow).ToString(), ClaimValueTypes.Integer64)); // Issue Date
        ...
        private UserResponseLogin GetResponseToken(string encodedToken, IdentityUser user, IEnumerable<Claim> claims)
        {
            return new UserResponseLogin
            {
                ...
                ExpiresIn = TimeSpan.FromHours(_appSettings.ExpiryHour).TotalSeconds,
